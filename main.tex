...
\figref{fig:Example} puts the two  profiling reports extracts side by side : the one on the left was before introducing the modified primitive, and the one on the right was once the new results were taken into account.
...
\begin{figure}
    \begin{center}
		...
		\noindent \scriptsize \begin{tabular}{l | l}
		Existing profiler report & Accurate profiler report \\		
		\midrule
		10305 samples in the VM	  & 10368 samples in the VM	\\	
		10007 samples in generated vm code & 10067 samples in generated vm code\\ 
		298 samples in vanilla vm code & 301 samples in vanilla vm code\\
		& \\
		\% of generated vm code (\% of total)(samples) & \% of generated vm code (\% of total)(samples) \\
		46.55\%    (45.08\%)	Integer$>>$benchFib	(4658)	(46.55\%) & 47.03\%    (45.58\%)	Integer{$>>$}benchFib	(4735)	(47.03) \\
	     & \MyIndent 57.95\% 1-$>$23	(2744)	(57.95\%)\\
	     & \MyIndent 5.89\%	24-$>$30	(279)	(63.84\%)\\
	     & \MyIndent 2.89\%	30-$>$31	(137)	(66.74\%)\\
	     & \MyIndent 8.24\%	31-$>$34	(390)	(74.97\%)\\
	     & \MyIndent 3.38\%	34-$>$35	(160)	(78.35\%)\\
	     & \MyIndent 6.59\%	35-$>$36	(312)	(84.94\%)\\
	     & \MyIndent 3.06\%	36-$>$38	(145)	(88.00\%)\\
	     & \MyIndent 12.00\%	38-$>$6667	(568)	(100.0\%)\\
		\end{tabular}
...
As this kind of tool is typically used to know where to boost performance, the existing VMprofiler was suffering from a small problem : it could not provide accurate profiling data for large native code functions. Indeed, it could report that most of the execution time was spent in a function, but not \textit{where} in this function the time was spent.
This problem was growing importance as more and more optimisations were performed by the JIT ; inlining, especially, makes the jitted functions larger, and thus, harder to profile accurately.

This paper describes a way to address this problem : an API used for debugging purposes offers a mapping between machine code pc and bytecode pc. This mapping can then be used to determine bytecode ranges in a large native code function, and thus identify how many samples were included in one or the other range.\\

Now, the VMProfiler can provide accurate profiling statistical reports.
ADD IMPACT Further improvements are currently being considered : 
\begin{itemize}
	...
	\item Sometimes, customers running a Windows environment request profiling, yet the VMProfiler is currently available for Mac and Linux only. The VMProfiler could then be implemented for Windows to tackle this problem.
\end{itemize}
